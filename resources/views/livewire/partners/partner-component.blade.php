...
<div id="content" class="bg-blue-50 mt-24 w-full min-w-ful mx-5 pb-5">
    ...
    <div class="border border-blue-800 mx-5 bg-white shadow rounded-lg overflow-hidden ">
        <table class=" bg-white table-fixed text-left border-b mx-2">
            <thead class="border-b  bg-green-50">
                <tr>
                    <th class=" py-2 w-1/2">Apellido</th>
                    <th class=" py-2 w-1/4">Nombre</th>
                    <th class=" py-2 w-1/8 text-center">DNI</th>
            ...
            <tbody>
                @foreach($partners as $partner)
                    <tr>
                        <td class="border-b px-4 py-3 text-sm text-blue-900">{{$partner->last_name}}</td>
                        <td class="border-b px-4 py-3 text-sm text-blue-900">{{$partner->name}}</td>
                        <td class="border-b px-4 py-3 text-center text-sm text-blue-900">{{$partner->DNI}}</td>
                        <td class="border-b px-4 py-3 flex justify-center flex flex-wrap">
                            @component('components/buttons.btn_edit')
                                @slot('function', "edit($partner->id)")
                            @endcomponent
                            @component('components/buttons.btn_delete')
                                @slot('function', "deleteModal($partner->id)")
                            @endcomponent
                        </td>
                    </tr>
                @endforeach
    ...
    @if($isOpen)
        @include('livewire.partners/modals/partner-create')
    @endif
    @if($isOpenDelete)
        @include('livewire.partners/modals/partner-delete')
    @endif
</div>
