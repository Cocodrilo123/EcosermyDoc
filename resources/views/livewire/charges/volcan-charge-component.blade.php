...
<div id="content" class="bg-blue-50 mt-24 w-full min-w-ful mx-5 pb-5">
    ...
    <div class="border border-blue-800 mx-5 bg-white shadow rounded-lg overflow-hidden ">
        <table class=" bg-white table-fixed text-left border-b ">
            ...
            <tbody>
                @foreach($volcanCharges as $volcanCharge)
                    <tr>
                        <td class="border-b px-3 py-3 text-center text-blue-900">{{$volcanCharge->id}}</td>
                        <td class="border-b px-4 py-3 text-sm text-blue-900">{{$volcanCharge->name}}</td>
                        <td class="border-b px-4 py-3 text-xs text-blue-900">{{$volcanCharge->description}}</td>
                        <td class="border-b px-4 py-3 flex justify-center flex flex-wrap">
                            @component('components/buttons.btn_edit')
                                @slot('function', "edit($volcanCharge->id)")
                            @endcomponent
                            @component('components/buttons.btn_delete')
                                @slot('function', "deleteModal($volcanCharge->id)")
                            @endcomponent
                        </td>
                    </tr>
                @endforeach
