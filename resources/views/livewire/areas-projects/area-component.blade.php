        ...
        <table class=" bg-white table-fixed text-left border-b ">
            ...
            <tbody>
                @foreach($areas as $area)
                    <tr>
                        <td class="border-b px-3 py-3 text-center text-blue-900">{{$area->id}}</td>
                        <td class="border-b px-4 py-3 text-sm text-blue-900">{{$area->name}}</td>
                        <td class="border-b px-4 py-3 text-xs text-blue-900">{{$area->description}}</td>
                        <td class="border-b px-4 py-3 flex justify-center flex flex-wrap">
                            @component('components/buttons.btn_edit')
                                @slot('function', "edit($area->id)")
                            @endcomponent
                            @component('components/buttons.btn_delete')
                                @slot('function', "deleteModal($area->id)")
                            @endcomponent
                        </td>
                    </tr>
                @endforeach
